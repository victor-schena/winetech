$(document).ready(function () {
  ...
  $("#searchName").blur(function () {
    ...
    $.post('/FornecedorPessoaFisica/Search', { Nome: $("#searchName").val() },
      function (data) {
        ...
        if (data !== [] && data.length > 0) {
          ...
          for (var i = 0; i < data.length; i++) {
            if (data[i].PapelPessoaId === 2 && data[i].TipoPessoaId === 1) {
            ...
            if (data[i].PapelPessoaId === 2 && data[i].TipoPessoaId === 2) {
          ...
          //exibir msg na tela
          mySelect.append($('<option></option>').val(0).text("Fornecedor não encontrado!"));
      ...
      $.post('/Produtos/Search', { Nome: $("#searchProduct").val() },
        function (data) {
          ...
          if (data !== [] && data.length > 0) {
            ...
            //ocultar loading
            for (var i = 0; i < data.length; i++) {
              var myOptions = { val: data[i].Nome + " - " + data[i].CustoUnitario.toLocaleString('pt-BR', { minimumFractionDigits: 2, style: 'currency', currency: 'BRL' }) };
              $.each(myOptions, function (val, text) {
                mySelect.append(
                  $('<option></option>').val(data[i].Id).html(text)
                );
  ...
  $("#enviarProduto").click(
    function enviarClienteItem() {
      $.post('/PedidoCompra/AddItem', { PedidoId: $('#PedidoId').val(), UserId: $('#User').val(), PessoaId: $('#selectClient').val(), ProdutoId: $('#selectProduct').val(), Quantidade: $('#qtdeProduto').val() },
        function (data) {
          console.log(data);
          ...
          for (var i = 0; i < data.Produtos.length; i++) {
            ...
            tr.append("<td class='text-center'>" + data.Produtos[i].CustoUnitario.toLocaleString('pt-BR', { minimumFractionDigits: 2, style: 'currency', currency: 'BRL' }) + "</td>");
  ...
  $("#finalizarPedido").click(function () {
    $.post('/PedidoCompra/FinalizarPedido', { PedidoId: $('#PedidoId').val(), UserId: $('#User').val(), PessoaId: $('#selectClient').val() },
      function (data) {
